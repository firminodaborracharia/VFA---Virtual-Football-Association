...
| `npm run db:doctor` | Diagnóstico completo: mostra o erro **real** do PostgreSQL, confere se as tabelas existem e se há dados. Use sempre que algo falhar. |
...
### Quando algo dá errado com o banco

O Drizzle embrulha falhas numa mensagem `Failed query: select …` que mostra a
consulta, não o motivo. Dois recursos resolvem isso:

```bash
npm run db:doctor
```

Ele imprime como a `DATABASE_URL` foi interpretada (sem revelar a senha), testa
a conexão, mostra o erro real do PostgreSQL com o código, confere se todas as
tabelas existem e conta os registros — e sugere o que costuma resolver cada caso.

Além disso, `src/instrumentation.ts` desembrulha a cadeia de causas de qualquer
erro de servidor, então o terminal onde o `npm run dev` roda passa a mostrar o
motivo de verdade (`relation "seasons" does not exist`, `password authentication
failed`, `ENETUNREACH`) em vez da consulta.

Os três problemas mais comuns:

| Sintoma | Causa | Solução |
|---|---|---|
| `relation "…" does not exist` | Migrations não aplicadas neste banco. | `npm run db:migrate` |
| `ENETUNREACH` | A conexão **direta** do Supabase só aceita IPv6, e a maioria das redes domésticas não tem. | Use a string do **Transaction pooler** (host `…pooler.supabase.com`, porta `6543`). |
| `password authentication failed` | Senha com caractere especial não codificado na URL. | Redefina a senha no Supabase sem símbolos, ou codifique-a. |
...
que costuma bastar. Para não depender disso, o `vercel.json` do projeto já
chama `/api/cron/publish` uma vez por dia, às 3h UTC (meia-noite em Brasília):
...
**Uma vez por dia é o limite do plano gratuito da Vercel.** Tentar
`*/15 * * * *` no plano Hobby faz o deploy falhar com "Hobby accounts are
limited to daily cron jobs". No plano Pro, troque para o intervalo que quiser.

Defina `CRON_SECRET` nas variáveis de ambiente se quiser proteger a rota: a
Vercel envia esse valor automaticamente no cabeçalho `Authorization` das
chamadas de cron, e é exatamente isso que a rota confere.
